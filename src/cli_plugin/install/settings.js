...
const LATEST_SOM_PLUGIN_BASE_URL = 'https://packages-dev.som.com';

function generateUrls({ version, plugin, somVersion }) {
  return plugin.includes('som')
    ? [plugin, generateSomPluginUrl(somVersion, plugin)]
...
function generateSomPluginUrl(version, plugin) {
  return `${LATEST_SOM_PLUGIN_BASE_URL}/pre-release/ui/dashboard/${plugin}-${version}-1.zip`;
}
...
export function parse(command, options, osdPackage) {
  const settings = {
    ...
    somVersion: osdPackage.som.version,
