...
| `healthcheck.server_not_ready_troubleshooting_link` | define the troubleshooting link in the not-ready server | URL to Som docs | a valid URL |
...
setup(core){
  ...
  core.healthCheck.register({
    name: 'custom-task',
    run: (ctx) => {
      console.log('Run from somCore starts' )
    },
    order: 1
    critical: false
  });
...
```console
journalctl -ru som-dashboard | grep healthcheck
```
