...
This script automates the process of updating the version, stage, and revision number across various files within the Som Dashboard repository. It uses standard shell commands (`sed`, `awk`, `grep`, etc.) for modifications.

## Purpose

The primary goal of this script is to ensure consistency when releasing a new version or build stage (alpha, beta, rc) of the Som Dashboard. It modifies key files to reflect the new version information, reducing the chance of manual errors.
...
3.  **Pre-update Checks:**
    ...
    - Reads the current `revision` from `package.json` (within the `"som"` object).
...
5.  **File Modifications:** Updates the version, stage, and/or revision in the following files using `sed`:
    ...
    - `package.json`: Updates `version` and `revision` within the `"som"` object.
    - `CHANGELOG.md`:
    ...
    - `.github/workflows/build_som_dashboard_with_plugins.yml`: Updates references like `yml@vX.Y.Z` to `yml@v<VERSION>-<STAGE>`.
    - `dev-tools/build-packages/base-packages-to-base/base-packages.Dockerfile`: Updates `_BRANCH=X.Y.Z` and `som-packages-to-base:X.Y.Z` references to use the new `<VERSION>`.
    ...
    - `src/core/server/rendering/__snapshots__/rendering_service.test.ts.snap`: Updates `"somVersion": "X.Y.Z"` to use the new `<VERSION>`.
