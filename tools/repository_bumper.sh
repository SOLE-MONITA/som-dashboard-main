#!/bin/bash
#
# Som Security Dashboards Plugin repository bumper (Pure Shell Version)
...
CURRENT_VERSION=""
TAG=false
SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE="${REPO_PATH}/.github/workflows/4_builderpackage_dashboard.yml"
...
RPM_CHANGELOG="${REPO_PATH}/dev-tools/build-packages/rpm/som-dashboard.spec"
...
pre_update_checks() {
  ...
  CURRENT_REVISION=$(sed -n '/"som": {/,/}/ s/^[[:space:]]*"revision"[[:space:]]*:[[:space:]]*"\([^"]*\)".*$/\1/p' "$PACKAGE_JSON" | head -n 1)
...
update_package_json() {
  if [ -f "$PACKAGE_JSON" ]; then
    ...
    # Use sed with address range to target lines within the "som": { ... } block
    # Update version in package.json
    if [[ "$CURRENT_VERSION" != "$VERSION" ]]; then
      log "Attempting to update version to $VERSION within 'som' object in $PACKAGE_JSON"
      # Note: This sed command assumes a specific formatting and might be fragile.
      # It looks for the block starting with a line containing "som": { and ending with the next line containing only }
      # Within that block, it replaces the value on the line starting with "version":
      sed_inplace "/\"som\": {/,/}/ s/^\([[:space:]]*\"version\"[[:space:]]*:[[:space:]]*\)\"[^\"]*\"/\1\"$VERSION\"/" "$PACKAGE_JSON"
      modified=true
    fi

    # Update revision in package.json
    if [[ "$CURRENT_REVISION" != "$REVISION" ]]; then
      log "Attempting to update revision to $REVISION within 'som' object in $PACKAGE_JSON"
      # Similar sed command for the revision line within the same block
      sed_inplace "/\"som\": {/,/}/ s/^\([[:space:]]*\"revision\"[[:space:]]*:[[:space:]]*\)\"[^\"]*\"/\1\"$REVISION\"/" "$PACKAGE_JSON"
      modified=true
    fi
...
update_changelog() {
  ...
  local changelog_header="## Som dashboard v${VERSION} - OpenSearch Dashboards ${OPENSEARCH_VERSION} - Revision "
  ...
  if grep -qE "$changelog_header_regex" "$changelog_file"; then
   ...
   # Create the new entry directly in the changelog using sed
    ...
    printf "## Som dashboard v%s - OpenSearch Dashboards %s - Revision %s\n\n### Added\n\n- Support for Som %s\n\n" "$VERSION" "$OPENSEARCH_VERSION" "$REVISION" "$VERSION" >>"$temp_file"
...
update_build_workflow() {
  log "Updating $(basename $SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE)..."

  if [ -f "$SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE" ]; then
    local modified=false
    ...
    if grep -qE '\.yml@[^"[:space:]]+' "$SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE"; then
      log "Pattern found in $(basename $SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE). Attempting update..."
      # If the pattern exists, perform the substitution
      sed_inplace -E "s/(\.yml@)[^\"[:space:]]+/\1${replacement}/g" "$SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE"
      modified=true
    else
      log "Pattern not found in $(basename $SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE). Skipping update."
    fi

    if [[ $modified == true ]]; then
      log "Successfully updated references to @${replacement} in $(basename "$SOM_DASHBOARD_PLUGINS_WORKFLOW_FILE")"
    fi
  fi
}
...
update_base_package_dockerfile() {
  ...
  if [ -f "$DOCKERFILE_FOR_BASE_PACKAGES" ]; then
    ...
    # Update all occurrences of som-packages-to-base:x.y.z with som-packages-to-base:$VERSION
    sed_inplace -E "s/(som-packages-to-base:)$VERSION_PATTERN/\1${VERSION}/g" "$DOCKERFILE_FOR_BASE_PACKAGES" && modified=true
...
update_rendering_service_test_snap() {
  ...
  if [ -f "$rendering_service_test_snap" ]; then
    ...
    # Update all occurrences of ("somVersion": "x.y.z",) with ("somVersion": "$VERSION",)
    # Define the pattern regex
    local pattern_regex="(\"somVersion\": \")$VERSION_PATTERN"
    # Check if the pattern exists in the file
...
update_healthcheck_server_not_ready_troubleshooting_link() {
  ...
  if [ -f "$file" ]; then
    ...
    if [[ "$current_version_minor" != "$version_minor" ]]; then
      # Update all occurrences of ("somVersion": "x.y.z",) with ("somVersion": "$VERSION",)
      # Define the pattern regex
      local pattern_regex="(https://documentation.som.com/)$current_version_minor"
      # Check if the pattern exists in the file
...
update_rpm_changelog() {
  ...
  local changelog_entry="* $rpm_date support <info@som.com> - $VERSION"
  local more_info_entry="- More info: https://documentation.som.com/current/release-notes/release-$(echo $VERSION | tr '.' '-').html"

  # Check if entry already exists
  if grep -q "* .* support <info@som.com> - $VERSION" "$RPM_CHANGELOG"; then
    log "RPM changelog entry for version $VERSION already exists. Updating date..."
    # Update existing entry date
    sed -i "s/^\* .* support <info@som.com> - $VERSION$/$changelog_entry/" "$RPM_CHANGELOG"
...
update_deb_changelog() {
  ...
  local changelog_header="som-dashboard ($package_version) stable; urgency=low"
  local more_info_entry="  * More info: https://documentation.som.com/current/release-notes/release-$(echo $VERSION | tr '.' '-').html"
  local maintainer_line=" -- Som, Inc <info@som.com>  $deb_date"

  # Escape parentheses and dots for grep and sed patterns
  local escaped_package_version=$(echo "$package_version" | sed 's/[().]/\\&/g')

  # Check if entry already exists
  if grep -q "som-dashboard ($package_version)" "$DEB_CHANGELOG"; then
    ...
    sed -i "/som-dashboard ($escaped_package_version)/,/^ *-- Som, Inc/ s|^ *-- Som, Inc <info@som.com> .*|$maintainer_line|" "$DEB_CHANGELOG"
