...
PACKAGE=""
# Container name
CONTAINER_NAME="som-dashboard"
# Files to check
FILES="/etc/som-dashboard/opensearch_dashboards.yml /usr/share/som-dashboard"
# Owner of the files
FILE_OWNER="som-dashboard"
...
# Check if files exist and are owned by som-dashboard
files_exist() {
...
check_opensearch_dashboard_yml() {
  ...
  diff_opensearch_dashboard_yml=$(docker exec $CONTAINER_NAME diff /etc/som-dashboard/opensearch_dashboards.yml /tmp/opensearch_dashboards.yml)
  ...
  echo $(docker exec $CONTAINER_NAME diff /etc/som-dashboard/opensearch_dashboards.yml /tmp/opensearch_dashboards.yml)
...
check_metadata_deb() {
  ...
  metadataVersion=$(docker exec $CONTAINER_NAME apt show som-dashboard | grep Version | awk '{print $2}')
  metadataPackage=$(docker exec $CONTAINER_NAME apt show som-dashboard | grep Package | awk '{print $2}')
  metadataStatus=$(docker exec $CONTAINER_NAME apt show som-dashboard | grep Status)
...
check_metadata_rpm() {
  metadataVersion=$(docker exec $CONTAINER_NAME rpm -q --qf '%{VERSION}-%{RELEASE}' som-dashboard)
  metadataPackage=$(docker exec $CONTAINER_NAME rpm -q --qf '%{NAME}' som-dashboard)
...
help() {
  ...
  echo
  echo "    -p, --package <path>       Set Som Dashboard rpm package name,which has to be in the <repository>/dev-tools/test-packages/<DISTRIBUTION>/ folder."
  echo
  exit $1
}
