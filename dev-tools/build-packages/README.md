...
- The base of Som Dashboard, generated by running `yarn build --linux --skip-os-packages --release` or `yarn build --linux-arm --skip-os-packages --release` for arm packages
- The build of each plugin in `som-dashboard-plugins` repo
- The build of the `som-security-dashboards-plugin` repo
- The build of the `som-dashboards-reporting ` repo
- The build of the `som-dashboard-security-analytics ` repo
...
- `-a`, `--app`: URL or path to the zip that contains the `som-dashboard-plugins` plugins build.
- `-b`, `--base`: URL or path to the zip that contains the `som-dashboard build`.
- `-s`, `--security`: URL or path to the zip that contains the `som-security-dashboards-plugin` build.
- `-rp`, `--reportPlugin`: URL or path to the zip that contains the `som-dashboard-reporting` build.
- `-sa`, `--securityAnalytics`: URL or path to the zip that contains the `som-dashboard-security-analytics` build.
...
> [!WARNING]
> To build `arm` packages, you need to run the script in an arm machine, and use an arm build of the som-dashboard base with `-b`

Example:

```bash
bash build-packages.sh \
    --app file:///home/user/packages/som-package.zip \
