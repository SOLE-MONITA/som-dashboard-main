...
mkdir -p ${tmp_dir}/som-dashboard-base
cd ${tmp_dir}/som-dashboard-base
log "Extracting base tar.gz..."
tar -zxf ${out_dir}/som-dashboard-$version-$revision-linux-$architecture.tar.gz
log "Preparing the package..."
jq '.som.revision="'${revision}'"' package.json > pkgtmp.json && mv pkgtmp.json package.json
...
cd ..
tar -czf som-dashboard.tar.gz som-dashboard-base
...
directory_base="/usr/share/som-dashboard"
