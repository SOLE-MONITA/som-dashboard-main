...
# Foundation.
Summary:     Som dashboard is a user interface and visualization tool for security-related data. Documentation can be found at https://documentation.som.com/current/getting-started/components/som-dashboard.html
Name:        som-dashboard
Version:     %{_version}
Release:     %{_release}
License:     GPL
...
buildroot:   %{_tmppath}/%{name}-%{version}-%{release}-som-dashboard-%(%{__id_u} -n)
Vendor:      Som, Inc <info@som.com>
Packager:    Som, Inc <info@som.com>
...
%global DASHBOARD_FILE som-dashboard.tar.gz
...
%description
Som dashboard is a user interface and visualization tool for security-related data. This Som central component enables exploring, visualizing, and analyzing the stored security alerts generated by the Som server. Som dashboard enables inspecting the status and managing the configurations of the Som cluster and agents as well as creating and managing users and roles. In addition, it allows testing the ruleset and making calls to the Som API. Documentation can be found at https://documentation.som.com/current/getting-started/components/som-dashboard.html
...
sed -i 's/OSD_NODE_OPTS_PREFIX/OSD_PATH_CONF="\/etc\/som-dashboard" OSD_NODE_OPTS_PREFIX/g' "som-dashboard-base/bin/opensearch-dashboards"
sed -i 's/OSD_USE_NODE_JS_FILE_PATH/OSD_PATH_CONF="\/etc\/som-dashboard" OSD_USE_NODE_JS_FILE_PATH/g' "som-dashboard-base/bin/opensearch-dashboards-keystore"
...
mv som-dashboard-base/config/node.options %{buildroot}%{CONFIG_DIR}
mv som-dashboard-base/config/opensearch_dashboards.yml %{buildroot}%{CONFIG_DIR}
cp som-dashboard-base/VERSION.json %{buildroot}%{INSTALL_DIR}

mv som-dashboard-base/* %{buildroot}%{INSTALL_DIR}

# Set custom welcome styles

mkdir -p %{buildroot}%{INSTALL_DIR}/config

cp %{buildroot}%{INSTALL_DIR}/etc/services/som-dashboard.service %{buildroot}/usr/lib/systemd/system/som-dashboard.service
cp %{buildroot}%{INSTALL_DIR}/etc/services/default %{buildroot}/etc/default/som-dashboard

chmod 644 %{buildroot}/usr/lib/systemd/system/som-dashboard.service
chmod 640 %{buildroot}/etc/default/som-dashboard
...
chown root:root %{buildroot}/usr/lib/systemd/system/som-dashboard.service

find %{buildroot}%{INSTALL_DIR}/plugins/som/ -exec chown %{USER}:%{GROUP} {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/som/ -type f -perm 644 -exec chmod 640 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/som/ -type f -perm 755 -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/som/ -type d -exec chmod 750 {} \;
find %{buildroot}%{INSTALL_DIR}/plugins/som/ -type f -perm 744 -exec chmod 740 {} \;

# -----------------------------------------------------------------------------

%pre
# Create the som-dashboard group if it doesn't exists
if [ $1 = 1 ]; then
  ...
  # Create the som-dashboard user if it doesn't exists
  if ! getent passwd %{USER} > /dev/null 2>&1; then
    useradd -g %{GROUP} -G %{USER} -d %{INSTALL_DIR}/ -r -s /sbin/nologin som-dashboard
  fi
fi
# Stop the services to upgrade the package
if [ $1 = 2 ]; then
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1 && systemctl is-active --quiet som-dashboard > /dev/null 2>&1; then
    systemctl stop som-dashboard.service > /dev/null 2>&1
    touch %{INSTALL_DIR}/som-dashboard.restart
  # Check for SysV
  elif command -v service > /dev/null 2>&1 && service som-dashboard status 2>/dev/null | grep "is running" > /dev/null 2>&1; then
    service som-dashboard stop > /dev/null 2>&1
    touch %{INSTALL_DIR}/som-dashboard.restart
  fi
fi
...
rm -f /usr/share/som-dashboard/VERSION
...
%preun
if [ $1 = 0 ];then # Remove
  echo -n "Stopping som-dashboard service..."
...
if [ $1 = 0 ];then
  ...
  # Remove the som-dashboard user if it exists
  if getent passwd %{USER} > /dev/null 2>&1; then
    userdel %{USER} >/dev/null 2>&1
  fi
  # Remove the som-dashboard group if it exists
  if command -v getent > /dev/null 2>&1 && getent group %{GROUP} > /dev/null 2>&1; then
  ...
  # Remove /etc/som-dashboard and /usr/share/som-dashboard dirs
  rm -rf %{INSTALL_DIR}
  if [ -d %{PID_DIR} ]; then
    rm -rf %{PID_DIR}
  fi
...
if [ -f %{INSTALL_DIR}/som-dashboard.restart ]; then
  rm -f %{INSTALL_DIR}/som-dashboard.restart
  if command -v systemctl > /dev/null 2>&1 && systemctl > /dev/null 2>&1; then
    systemctl restart som-dashboard.service > /dev/null 2>&1
  # Check for SysV
  elif command -v service > /dev/null 2>&1; then
    service som-dashboard restart > /dev/null 2>&1
  fi
...
%config(noreplace) %attr(0750, %{USER}, %{GROUP}) "/etc/default/som-dashboard"
...
%attr(644, root, root) "/usr/lib/systemd/system/som-dashboard.service"

%changelog
* Thu Dec 18 2025 support <info@som.com> - 5.0.0
- More info: https://documentation.som.com/current/release-notes/release-5-0-0.html
* Wed Dec 17 2025 support <info@som.com> - 4.14.2
- More info: https://documentation.som.com/current/release-notes/release-4-14-2.html
* Wed Nov 12 2025 support <info@som.com> - 4.14.1
- More info: https://documentation.som.com/current/release-notes/release-4-14-1.html
...
* Wed May 21 2025 support <info@som.com> - 4.10.2
- More info: https://documentation.som.com/current/release-notes/release-4-10-2.html
...
* Wed Feb 21 2025 support <info@som.com> - 4.11.0
