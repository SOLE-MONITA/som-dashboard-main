...
clean() {
    ...
    rm -f ${current_path}/rpm/Docker/som-dashboard.spec
...
get_packages(){
  ...
  packages_names=("Som plugins" "Som Dashboard" "Security plugin"  "Report plugin" "Security analytics plugin")
...
build_rpm() {
  ...
  cp ./som-dashboard.spec ${dockerfile_path}
  run_with_retry docker build -t "${container_name}" "${dockerfile_path}" || return 1
  run_with_retry docker run -t --rm -v "${tmp_dir}/:/tmp:Z" -v "${output_dir}/:/output:Z" \
...
help() {
    ...
    echo "    -a,  --app <url/path>           Set the location of the .zip file containing the Som plugin."
    echo "    -b,  --base <url/path>          Set the location of the .tar.gz file containing the base som-dashboard build."
    echo "    -s,  --security <url/path>      Set the location of the .zip file containing the som-security-dashboards-plugin."
    echo "    -sa, --securityAnalytics <url/path>          Set the location of the .zip file containing the som-dashboard-security-analytics plugin."
    echo "    -rp, --reportPlugin <url/path>  Set the location of the .zip file containing the som-reporting-plugin."
