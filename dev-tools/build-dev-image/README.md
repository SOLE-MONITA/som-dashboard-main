# Som Dashboard Development Images

This directory contains tools and scripts for building Docker images for Som Dashboard development environment.
...
- **install-plugins.sh**: Script that installs Som plugins during build
...
docker build \
  ...
  --build-arg SOM_DASHBOARD_BRANCH=migrate-main-to-3.3.0 \
  --build-arg SOM_DASHBOARD_SECURITY_BRANCH=migrate-main-to-3.3.0.0 \
  --build-arg SOM_DASHBOARD_REPORTING_BRANCH=migrate-main-to-3.3.0.0 \
  --build-arg SOM_DASHBOARD_PLUGINS_BRANCH=migrate-main-to-3.3.0 \
  --build-arg SOM_DASHBOARD_ML_COMMONS_BRANCH=migrate-main-to-3.3.0 \
  --build-arg SOM_DASHBOARD_SECURITY_ANALYTICS_BRANCH=migrate-main-to-3.3.0 \
...
| `--som-branch` | `-w` | Som Dashboard branch | `migrate-main-to-3.3.0` |
| `--security-branch` | `-s` | Som Dashboard Security branch | `migrate-main-to-3.3.0.0` |
| `--reporting-branch` | `-r` | Som Dashboard Reporting branch | `migrate-main-to-3.3.0.0` |
| `--plugins-branch` | `-p` | Som Dashboard Plugins branch | `migrate-main-to-3.3.0` |
| `--ml-commons-branch` | `-ml` | Som Dashboard ML commons branch | `migrate-main-to-3.3.0` |
| `--security-analytics-branch` | `-sa` | Som Dashboard Security analystics branch | `migrate-main-to-3.3.0` |
...
./build-multiarch.sh --som-branch main --tag latest
...
./build-multiarch.sh --som-branch feature/new-feature --tag feature-test
```
...
docker build --platform linux/arm64 \
  ...
  -t quay.io/som/osd-dev:3.3.0-arm64 \
  -f wzd.dockerfile .
...
docker build --platform linux/amd64 \
  ...
  --build-arg OPENSEARCH_DASHBOARD_VERSION=3.3.0.0 \
  --build-arg SOM_DASHBOARD_BRANCH=main \
  -t quay.io/som/osd-dev:3.3.0-amd64 \
  -f wzd.dockerfile .
```
...
```bash
# Push single image
docker push quay.io/som/osd-dev:version
...
- **SOM_DASHBOARD_BRANCH**: Main Som Dashboard repository branch
- **SOM_DASHBOARD_SECURITY_BRANCH**: Security plugin branch
- **SOM_DASHBOARD_REPORTING_BRANCH**: Reporting plugin branch
- **SOM_DASHBOARD_PLUGINS_BRANCH**: Core plugins branch

## What the Image Contains

The built image includes:
- Node.js runtime environment
- Som Dashboard with specified branch
- Pre-installed Som plugins (security, reporting, core plugins)
...
docker pull quay.io/som/osd-dev:latest
```
...
```bash
# Feature development
./build-multiarch.sh --som-branch feature/my-feature --tag feature-test
